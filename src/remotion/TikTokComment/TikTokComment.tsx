...
export const TikTokComment: React.FC<TikTokCommentProps> = ({
  ...
  if (relativeFrame < animationDuration) {
    ...
    switch (animationType) {
      ...
      case "slideIn":
        ...
        animatedX = interpolate(progress, [0, 1], [width, x * width], {
      ...
      case "scaleIn":
        ...
        animatedScale = interpolate(progress, [0, 1], [0, scale], {
          easing: Easing.out(Easing.back(1.5)),
        });
        break;
  ...
  if (relativeFrame > totalDuration - exitDuration) {
    const exitProgress =
      (relativeFrame - (totalDuration - exitDuration)) / exitDuration;
    animatedOpacity *= interpolate(exitProgress, [0, 1], [1, 0.8], {
  ...
  return (
    <div style={commentStyle} className={`tiktok-comment ${className}`}>
      <Img
        src={imagePath}
        style={{
          width: "100%",
  ...
  return (
    <>
      {comments.map((comment, index) => (
        <TikTokComment key={`comment-${index}`} {...comment} />
      ))}
    </>
  );
};
