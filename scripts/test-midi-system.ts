...
async function testMidiSystem(): Promise<void> {
  ...
  try {
    ...
    firstNotes.forEach((note, index) => {
      ...
      console.log(
        `   Note ${index + 1}: ${noteName} (${frequency.toFixed(1)}Hz) - Vélocité: ${note.velocity.toFixed(2)} - Durée: ${note.duration.toFixed(2)}s`,
      );
    });
    console.log("");
    ...
    console.log(
      `   📋 Premiers fichiers: ${availableFiles.slice(0, 3).join(", ")}\n`,
    );

    // 6. Test d'un autre fichier pour vérifier le cache
    console.log("6. Test du cache avec un second fichier...");
    const secondFile =
      availableFiles.find((file) => file !== selectedFile) || availableFiles[0];
    ...
    console.log(
      `   📊 Accélération: ${(loadTime1 / Math.max(loadTime2, 1)).toFixed(1)}x\n`,
    );
    ...
    console.log(
      `Notes totales analysées: ${midiData.notes.length + secondMidiData.notes.length}`,
    );
